...
Example: Whether or not being in first class made someone more likely to survive from the Titanic incident

Potential confounder: women and children are more likely to be in first class
...
Ideas: 
1. if treatment effects are heterogenous, Z shock would only identify some causal effect of D on Y (only know effect of compliers)
...
Requirements:
3. Exclusion restriction path -> instrumental variable only affects exposure of interest
4. Z is correlated with D and thus correlated to Y. (Z to D is known as first stage, D to Y is second stage, Z to Y is known as reduced form)
...
1) Stable unit treatment value assumption -> potential outcomes for each person are unrelated to treatment status of other individuals

2) Independence assumption -> “as good as random assignment” assumption (IV is independent of potential outcomes and potential treatment assignments)

3) Exclusion restriction -> any effect of Z on Y must be via effect of Z on D

4) First stage -> Z has to have a statistically significant effect on average probability of treatment

5) Monotoicity assumption -> Instrument variable operate in same direction on all individual unit (so as to guarantee a weighted average of the underlying causal effects of the affected group
